...
无论如何，只要一个一账通实例决定对外公开自身的存在，用户就可以通过浏览器访问到该实例的登录界面。如果该实例选择开放注册，那么任何人都可以在此处使用邮箱或手机注册一个账号并登录。如若不然，用户只能选择请这个实例的管理员为自己手动添加一个账号，并通过管理员提供的链接绑定并激活账号。
...
添加账号时管理员必须为用户设置用户名，姓名以及登录密码、手机号、邮箱三者中至少一个，其他内容可根据需要添加。一个手机号或邮箱只能绑定一个账号。添加成功后，管理员可直接告诉用户密码，让其使用用户名和密码登录激活，也可以给其发送邀请激活链接。
管理员点击账号后的「邀请激活」即可将激活链接复制到剪切板。若管理员为用户填写了密码，手机号，邮箱三者中的多个，用户可根据链接自行选择激活方式。
...
管理员可点击账号后的「应用内权限」设置用户对应用的访问权限和应用内权限。
应用访问权限即使用该账号登录以后，能否在「我的应用」看到某个应用；应用内权限指使用该账号访问「我的应用」中的某个应用后，能看到该应用里的哪些内容。一账通可以实现对第三方应用内部各个功能的可见性的控制，但是这需要第三方应用自身对自己功能进行模块划分，并实现与一账通的交互。
也就是说，如果第三方应用对自己内部划分了区域，那么一账通可以控制用户可以看到哪些区域。如果第三方应用没有实现该功能，那么实际上应用内权限无法显示其作用（应用内权限可在 `应用权限管理`_ 添加）。

个人权限分为默认，是和否。初始状态为默认，此时权限结果来自于分组权限。若管理员为用户设置了个人权限，则权限结果来源于个人权限。个人权限的优先级最高，起决定作用。应用内权限同理，初始也为默认，即用户可访问应用的全部内容。
其中对应用的分组权限在「分组管理」或者「应用管理」中设置。
...
一账通默认提供部门、角色以及标签三种分组类型。用户可在这三个分类下自定义分组，也可以在已有分组下添加下级分组。

添加分组时，用户必须为分组填写名称。上级部门和可见范围根据需要选择。上级部门不选择时，默认为选中的分组。
分组可见性有以下几种：
...
不选择可见范围时，默认为所有人可见；若选择只对部分人可见，页面下方会新出现供选择可见范围的输入框。用户可根据账号和分组进行选择，此时不选择指定范围为所有人不可见。
若某一不可见分组下级分组可见，则该不可见分组会被迫可见。 
...
分组可见性计算比较复杂，存在以下几种特殊情况：

.. figure:: assets/分组可见性示例.*
   :align: center

+------------------------------+--------------------------------+-------------------------+-------------------------+----------------------------+
|分组A                         |  分组B                          |  成员c                  |     成员d                | 成员e                      |
+==============================+================================+=========================+==========================+===========================+
|所有人不可见                   | 所有人可见                      |可见分组B及其下级成员                                                             |
|                              |                                |可见分组A，不可见A下级成员                                                        |
+------------------------------+--------------------------------+-------------------------+--------------------------+---------------------------+
|所有人不可见                   | 仅组内成员可见（下属分组不可见）  |不可见分组A               | 可见分组B及其下级成员      |不可见分组A                 |
|                              |                                |不可见分组B               |可见分组A，不可见A下级成员   |不可见分组B                |
+------------------------------+--------------------------------+-------------------------+---------------------------+---------------------------+
|所有人不可见                   | 组内成员及其下属分组可见         | 不可见分组A              |可见分组B及其下级成员        |不可见分组A                 |
|                              |                                |不可见分组B               |可见分组A，不可见A下级成员   |不可见分组B                 |
+------------------------------+--------------------------------+-------------------------+---------------------------+---------------------------+
|仅组内成员可见（下属分组不可见） | 所有人可见                      | 可见分组A及其下级成员     | 可见分组B及其下级成员       |可见分组B及其下级成员       |
|                              |                                | 可见分组B及其下级成员     | 可见分组A，不可见A下级成员  |可见分组A，不可见A下级成员   |
+------------------------------+--------------------------------+-------------------------+---------------------------+---------------------------+
|仅组内成员可见（下属分组不可见） | 仅组内成员可见（下属分组不可见）  | 可见分组A及其下级成员     |  不可见分组A               |不可见分组A                |
|                              |                                |  不可见分组B             |   可见分组B及其下级成员     |不可见分组B                 |
+------------------------------+------------------------------- +--------------------------+--------------------------+----------------------------+
|仅组内成员可见（下属分组不可见） | 组内成员及其下属分组可见         | 可见分组A及其下级成员      | 不可见分组A               |不可见分组A                 |
|                              |                                | 不可见分组B               | 可见分组B及其下级成员      |不可见分组B                 |
+------------------------------+--------------------------------+--------------------------+--------------------------+----------------------------+
|组内成员及其下属分组可见        | 所有人可见                      |  可见分组A及其下级成员     |  可见分组A及其下级成员     |可见分组A，不可见A下级成员    |
|                              |                                |   可见分组B及其下级成员    |    可见分组B及其下级成员   |可见分组B及其下级成员        |
+------------------------------+--------------------------------+--------------------------+---------------------------+---------------------------+
|组内成员及其下属分组可见        |   仅组内成员可见（下属分组不可见）|   可见分组A及其下级成员    |    可见分组A及其下级成员    |不可见分组A                 |
|                              |                                |   不可见分组B             |    可见分组B及其下级成员    |不可见分组B                 |
+------------------------------+--------------------------------+--------------------------+----------------------------+---------------------------+
|组内成员及其下属分组可见        | 组内成员及其下属分组可见         |  可见分组A及其下级成员     |  可见分组A及其下级成员       |不可见分组A                 |
|                              |                                |  不可见分组B              |  可见分组B及其下级成员       |不可见分组B                 |
+------------------------------+--------------------------------+--------------------------+----------------------------+----------------------------+
...
管理员可在此页面设置分组对所有应用的权限，权限分为默认、是和否。分组对应用的权限初始为默认，此时该分组的权限来自于其上级分组。若其上级分组也为默认，则结果为没有权限。分组权限又包括应用访问权限和应用内权限。                    

应用访问权限指用户能否在「工作台」-「我的应用」中看到某一应用。若管理员在此将分组对某一应用的分组权限设置为是，在不受账号权限的影响时，则该分组下的成员在登录以后，都可以在「工作台」-「我的应用」中看到该应用。          
应用内权限指允许用户看到第三方应用内的哪些内容。管理员可在「应用管理」-「权限管理」为应用添加应用内权限。在一账通中，只需要设置应用内权限的名称，具体怎么给接入的第三方应用划分模块，需要第三方应用自己来完成。也就是说，一账通提供对第三方应用内部模块可见性的精细控制。若第三方应用没有实现此功能，则应用内权限不显示其作用。          
...
.. attention::
   如果A分组下存在B分组，B分组对某一应用的权限为默认时，B分组对该应用的权限表现继承自A分组；但是B分组对该应用的权限为是（或否）时，B分组对该应用的权限表现为是（或否）。
...
添加应用以后会自动生成一个用于访问应用的应用访问权限。即用户登录以后能否在「我的应用」看到某个应用。

管理员也可以自定义应用访问权限，来实现用户访问第三方应用后以不同的身份登录（不同的身份可能权限不同），或登录到第三方应用的不同页面。
自定义应用访问权限时要填写登录地址，用户名和登录密码。登录地址为域名，用户名和密码为登录第三方应用的身份。实现此功能需要下载第三方插件，并完成相关配置（见 “arkid 第三方账号管理” 文档）。这样，在 ArkID 工作台，点击应用，来到应用主页，选择登录，可见 ArkID 插件可选择账号密码填充。选中、确认登录即可。
...
应用内权限是指将应用的内部功能进行模块划分，来实现对用户可访问的模块进行限制。此功能需要第三方应用对自身的应用内权限进行定义，在一账通中添加相应的权限名称，通过权限ID实现两者的交互。
通俗地讲，一账通就如同插座，提供插口，可以控制每个插口是否有电。但是接入什么电器，接入的电器有什么功能，需要使用者自己决定并实施。
...
管理员可通过将账号加入黑白名单的方式来设置账号对应用的权限。白名单即拥有权限，黑名单没有权限。管理员可在结果名单查看有权限的账号和其权限来源。账号的权限优先级最高。这里的账号的权限与 `账号管理`_ 中的应用内权限一致。在「账号管理」部分设置权限以后，会显示在该部分相应应用的黑白名单。 
...
部门的权限与账号的权限类似，同样通过加入黑白名单的方式来实现权限的分配，不过此时添加的是部门，添加完成以后该设置对部门下的成员生效。
...
管理员点击添加新的子管理员组进入「添加子管理员」页面。
管理员需要选择要添加的用户，管理范围，并为子管理员分配权限。
...
管理范围包括所在分组及下级分组、特定账号及分组。管理员可对可管理的对象行使如下权力。
    ...
    - 分组内创建账号(前提是有创建用户的权限)
    - 管理分组内直属成员
    - 将已有成员移入该分组 (前提是有该成员的管理权限)
    - 将成员从分组内移除 (前提是有该账号的管理权限)

- 基础权限

    - 创建用户：子管理员能否在账号管理和分组管理创建用户。若想在分组创建用户还需要对分组有管理权限
    - 创建大类：在分组管理部分创建自定义分类
    - 创建应用：在应用管理创建应用。管理员可对自己创建应用的信息，权限进行管理，并且应用会显示在创建者的「我的应用」
    - 查看日志：查看操作日志
    - 公司基本信息配置、基础设施配置
    - 账号同步：目前只支持与钉钉同步。该同步表示将该一账通实例的所有账号进行同步
...
.. attention::
   为子管理员添加应用的权限后，该子管理员在「应用管理」页面只能看到其拥有权限的应用。
...
权限控制是一账通的核心功能，在这里对权限的计算进行介绍。
在一账通中权限计算的目标是“人”，即最终判断人对应用是否拥有权限。

判断某个账号对某个应用是否拥有权限时，首先会去检索账号的权限。账号的权限可通过「账号管理」的应用内权限和「应用管理」的账号的权限实现。账号的权限起决定作用，不受其他权限影响。

若未对账号的权限进行设置，则为默认，此时去校验账号所在的分组是否有权限。分组的权限可在「分组管理」的分组权限管理和「应用管理」的分组的权限设置。
分组的权限对其直属成员起作用，若未设置分组的权限，分组的权限也为默认，其权限继承自其上级分组的权限。顶层分组的权限为默认时，表现为没有权限。

事实上，「应用管理」的权限管理是对「账号管理」和「分组管理」中的权限管理进行了整合。
