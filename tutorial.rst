...
管理员可在此页面设置分组对所有应用的权限，权限分为默认、是和否，默认和否都表示没有权限。分组权限又包括应用访问权限和应用内权限。                    

应用访问权限指用户能否在「工作台」-「我的应用」中看到某一应用。若管理员在此将分组对某一应用的分组权限设置为是，则该分组下的成员及其下级分组成员在登录以后，都可以在「工作台」-「我的应用」中看到该应用。          
应用内权限指允许用户看到第三方应用内的哪些内容。管理员可在「应用管理」-「权限管理」为应用添加应用内权限。在一账通中，只需要设置应用内权限的名称，具体怎么给接入的第三方应用划分模块，需要第三方应用自己来完成。也就是说，一账通提供对第三方应用内部模块可见性的精细控制。           
...
管理员也可以自定义应用访问权限，来实现用户访问第三方应用后以不同的身份登录，或登录到第三方应用的不同页面。
...
管理员可通过将账号加入黑白名单的方式来设置账号对应用的权限。白名单即拥有权限，黑名单没有权限。管理员可在结果名单查看有权限的账号和其权限来源。账号的权限优先级最高。
