...
例如，拥有应用「充命名」和应用「猎聘」访问权限的账号 mei123 ,在登录以后可在「我的应用」页面看到这两个应用。若管理员为应用添加了访问链接，用户可直接点击应用访问。

.. figure:: assets/mei123的权限.*
   :align: center
...
.. attention::
   若某一不可见分组下级分组可见，则该不可见分组会被迫可见。此时被迫可见的分组只显示其分组名称，不显示下属成员。
...
应用内权限指允许用户看到第三方应用内的哪些内容。管理员可在「应用管理」-「权限管理」为应用添加应用内权限。
在一账通中，只需要设置应用内权限的名称，具体怎么给接入的第三方应用划分模块，需要第三方应用自己来完成。也就是说，一账通提供对第三方应用内部模块可见性的精细控制。若第三方应用没有实现此功能，则应用内权限不显示其作用。          
...
应用内权限是指将接入一账通的第三方应用的内部功能进行模块划分，一账通可实现对用户可访问的模块进行限制。此功能需要第三方应用对自身的应用内权限进行定义，在一账通中添加相应的权限名称，通过权限ID实现两者的交互。

通俗地讲，一个第三方应用如同一栋房子，有许多房间，一账通是掌管各个房间钥匙的管家。有住户进来（用户访问第三方应用）时，一账通可以决定给住户哪吧钥匙。前提是，这栋房子有不同的房间，并且和一账通协同配置好了房间的锁和钥匙。
...
.. attention::
   为子管理员添加应用权限后，该子管理员在「应用管理」页面只能看到其拥有权限的应用。
